...
/** Tokenize a numeric rule prefix when present. */
function applyNumberedRuleSidebarLabel(
...
/**
 * Tokenize a TypeDoc runtime-kind prefix when present.
 *
 * @returns `true` when the runtime prefix was applied.
 */
function hasAppliedRuntimeSidebarLabel(
    link: HTMLAnchorElement,
    linkLabel: string,
    mutations: SidebarLabelMutation[]
): boolean {
...
/**
 * Tokenize one eligible sidebar link.
 *
 * @param link - Candidate sidebar link.
 * @param mutations - Mutable collection receiving restorable label changes.
 */
function processSidebarLink(
    ...
    if (hasAppliedRuntimeSidebarLabel(link, linkLabel, mutations)) return;

    applyNumberedRuleSidebarLabel(link, linkLabel, mutations);
}
