...
Require config variables array keeps actionlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `actionlint/require-actionlint-config-variables-array`.
...
The rule reports missing required actionlint config structure for require config variables array.
...
export default [
 ...
 {
  rules: {
   "actionlint/require-actionlint-config-variables-array": "error",
  },
 },
];
```
