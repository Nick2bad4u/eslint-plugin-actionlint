...
Require paths object keeps actionlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `actionlint/require-actionlint-paths-object`.
...
The rule reports missing required actionlint config structure for require paths object.
...
export default [
 ...
 {
  rules: {
   "actionlint/require-actionlint-paths-object": "error",
  },
 },
];
```
