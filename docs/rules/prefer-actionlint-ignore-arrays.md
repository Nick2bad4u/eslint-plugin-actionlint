...
Prefer ignore arrays keeps actionlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `actionlint/prefer-actionlint-ignore-arrays`.
...
The rule reports older or less explicit configuration shapes and recommends the form used by the shared presets.
...
export default [
 ...
 {
  rules: {
   "actionlint/prefer-actionlint-ignore-arrays": "error",
  },
 },
];
```
