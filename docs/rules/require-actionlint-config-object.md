...
Require config object keeps actionlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `actionlint/require-actionlint-config-object`.
...
The rule reports missing required actionlint config structure for require config object.
...
export default [
 ...
 {
  rules: {
   "actionlint/require-actionlint-config-object": "error",
  },
 },
];
```
