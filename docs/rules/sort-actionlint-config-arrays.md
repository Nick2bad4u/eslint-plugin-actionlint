...
Sort config arrays keeps actionlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `actionlint/sort-actionlint-config-arrays`.
...
The rule reports arrays or object keys that are not sorted, keeping actionlint config review diffs stable.
...
export default [
 ...
 {
  rules: {
   "actionlint/sort-actionlint-config-arrays": "error",
  },
 },
];
```
