...
Require self hosted runner labels array keeps actionlint configuration files predictable when they are reviewed beside ESLint configuration.
...
- Files matched by the bridge rule or the configuration preset that enables `actionlint/require-actionlint-self-hosted-runner-labels-array`.
...
The rule reports missing required actionlint config structure for require self hosted runner labels array.
...
export default [
 ...
 {
  rules: {
   "actionlint/require-actionlint-self-hosted-runner-labels-array": "error",
  },
 },
];
```
