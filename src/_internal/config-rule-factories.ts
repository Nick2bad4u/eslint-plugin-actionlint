import type { ActionlintConfigReference } from "./actionlint-config-references.js";
...
type ConfigRuleDefinition = Readonly<{
    ...
    configs: readonly ActionlintConfigReference[];
    description: string;
    name: string;
    recommended: boolean;
}>;
...
export const hasConfigProperty = (
    ...
    collectPropertyMatches(sourceText, yamlTopLevelPropertyPattern).includes(
        propertyName
    ) ||
    collectPropertyMatches(sourceText, jsObjectPropertyPattern).includes(
        propertyName
    ) ||
...
export const createUnknownPropertiesRule = (
    ...
    createConfigTextRule({
        ...
        check: (sourceText) => {
            ...
            const properties = collectPropertyMatches(
                sourceText,
...
export const createConfigTextRule = (
    ...
    createTypedRule<MessageIds, Options>({
        ...
        meta: {
            ...
            messages: { configProblem: "Actionlint config: {{message}}" },
            schema: [],
            type: "problem",
        },
