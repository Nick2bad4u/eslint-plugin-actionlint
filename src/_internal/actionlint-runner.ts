...
import type {
    ...
    SerializableActionlintLintOptions,
...
const lintResultCache = new Map<string, SerializableActionlintResult>();
let actionlintWorker: null | Worker = null;
...
const resetWorker = (): void => {
    const workerToTerminate = actionlintWorker;
    ...
    actionlintWorker = null;
};

const getWorker = (): Worker => {
    if (actionlintWorker === null) {
        actionlintWorker = createWorker();
        actionlintWorker.unref();
        actionlintWorker.once("error", () => {
            actionlintWorker = null;
        });
        actionlintWorker.once("exit", () => {
            actionlintWorker = null;
        });
    }
    return actionlintWorker;
};

const readWorkerResponse = (
    response: ActionlintWorkerResponse | undefined
): SerializableActionlintResult => {
...
export const runActionlintSynchronously = (
    ...
    if (waitResult === "timed-out") {
        ...
        resetWorker();
        throw new Error(
            "Timed out while waiting for the Actionlint worker to finish."
        );
    }
    if (Atomics.load(signal, 0) !== WORKER_DONE_STATE) {
        port1.close();
        throw new Error("Actionlint worker did not enter a completed state.");
    }
    const workerMessage = receiveMessageOnPort(port1);
    port1.close();
    const result = readWorkerResponse(
        safeCastTo<ActionlintWorkerResponse | undefined>(workerMessage?.message)
    );
